...
const OtpVerificationForForgetPassword = () => {
  ...
  const userType =
    new URLSearchParams(location.search).get("type") || "student";
  ...
  const handleResend = () => {
    ...
    if (!userEmail) {
      ...
      toast.error(
        "Email address not found. Please go back to the previous step."
      );
      return;
    }
    ...
    fetch(
      "https://ak6ymkhnh0.execute-api.us-east-1.amazonaws.com/dev/api/verify-email",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export default OtpVerificationForForgetPassword;
